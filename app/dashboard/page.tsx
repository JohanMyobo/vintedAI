...
export default function DashboardPage() {
  ...
  const handleDelete = useCallback(async (id: string) => {
    if (!confirm('Supprimer cette annonce du dashboard ?')) return
    const res = await fetch(`/api/annonces/${id}`, { method: 'DELETE' })
    if (res.ok) setAnnonces(prev => prev.filter(a => a.id !== id))
  }, [])

  const filtered = filtre === 'Toutes' ? annonces : annonces.filter(a => a.statut === FILTRE_MAP[filtre])
        ...
        {isLoaded && isSignedIn && !loading && (
          <>
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/" className="px-4 py-2 text-sm rounded-lg bg-[#1D9E75] text-white font-semibold hover:bg-[#178a64] transition-colors">
                + Générer
              </Link>
            </div>
              ...
              {FILTRES.map(f => (
                <button key={f} onClick={() => setFiltre(f)}
                  className={`px-3 py-1.5 rounded-full text-sm font-medium whitespace-nowrap transition-colors ${
                    filtre === f ? 'bg-[#1D9E75] text-white' : 'bg-white border border-gray-200 text-gray-600 hover:border-[#1D9E75]'
                  }`}>
                  {f}
                  {f !== 'Toutes' && <span className="ml-1 text-xs opacity-70">{annonces.filter(a => a.statut === FILTRE_MAP[f]).length}</span>}
                </button>
              ))}
            </div>
            ...
            {filtered.length === 0 ? (
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-10 text-center">
                ...
                {filtre === 'Toutes' && <p className="text-sm text-gray-400 mt-1">Génère une annonce et ajoute-la au dashboard</p>}
              </div>
            ) : (
              <div className="space-y-3">
                {filtered.map(annonce => (
                  <AnnonceCard key={annonce.id} annonce={annonce} onUpdate={handleUpdate} onDelete={handleDelete} />
                ))}
              </div>
            )}
          </>
